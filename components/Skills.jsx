...
function SkillCard({ icon, name }) {
  return (
    ...
    >
      ...
      <span className="text-sm font-medium text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{name}</span>
    </motion.div>
  );
}
    ...
    <section id="skills" className="relative bg-white dark:bg-slate-950 px-4 py-24 overflow-hidden transition-colors duration-300 gpu-accelerated">
      ...
      <div className="mx-auto w-full max-w-6xl relative">
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="p-3 rounded-2xl bg-indigo-500/5 dark:bg-indigo-500/10 text-2xl">
                    {group.icon}
                  </div>
                  <div>
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-16 rounded-2xl border border-slate-200 dark:border-indigo-500/20 bg-slate-50 dark:bg-indigo-500/5 p-6 text-center"
        >
          <p className="text-slate-600 dark:text-slate-300 text-sm">
            <span className="text-indigo-600 dark:text-indigo-400 font-semibold mr-2">Focus:</span> 
            Currently deepening expertise in <span className="text-slate-900 dark:text-white font-medium">Deep Learning</span> and <span className="text-slate-900 dark:text-white font-medium">Big Data Technologies (Spark)</span> to solve larger-scale analytical challenges.
          </p>
        </motion.div>
      </div>
    </section>
